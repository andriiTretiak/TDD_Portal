    ...
    response: {
        data: {
            validationErrors: {
                ...
                image: 'Only PNG and JPG files allowed'
            }
        }
    }
};
...
describe('UserPage', () => {
    ...
    describe('ProfileCard Interactions', () => {
        ...
        it('returns to none edit mode after clicking cancel', async () => {
            const { queryByText } = await setupForEdit();
        ...
        it('calls updateUser api with userId', async () => {
            const { queryByText } = await setupForEdit();
            apiCalls.updateUser = jest.fn().mockResolvedValue(mockSuccessUpdateUser);
        ...
        it('returns to last updated displayName when display name changed for another time but cancelled', async () => {
            const { queryByText, container } = await setupForEdit();
        ...
        it('enable save button after updateUser api call success', async () => {
            const { queryByText, container } = await setupForEdit();
            let displayInput = container.querySelector('input');
            fireEvent.change(displayInput, {target: {value: 'display1-update'}});
            apiCalls.updateUser = jest.fn().mockResolvedValue(mockSuccessUpdateUser);
        ...
        it('displays the selected image in edit mode', async () => {
            ...
            const file = new File(['demmy content'], 'example.png', {type: 'image/png'});
        ...
        it('returns the to the original image even the new image is added to upload box but cancelled', async () => {
            const { queryByText, container } = await setupForEdit();
        ...
        it('removes validation error for displayName when user change displayName', async () => {
            const { queryByText, container } = await setupForEdit();
            apiCalls.updateUser = jest.fn().mockRejectedValue(mockFailUpdateUser);
            ...
            fireEvent.click(saveButton);
            await waitForDomChange();
            const displayName = container.querySelectorAll('input')[0];
            fireEvent.change(displayName, {target: {value: 'new-display-name'}});
            const errorMessage = queryByText('It must have minimum 4 and maximum 255 characters');
        ...
        it('update localStorage after updateUser api call success', async () => {
            ...
            fireEvent.click(saveButton);
            await waitForDomChange();
            const storedUserData = JSON.parse(localStorage.getItem('portal-auth'));
            expect(storedUserData.displayName).toBe(mockSuccessUpdateUser.data.displayName);
            expect(storedUserData.image).toBe(mockSuccessUpdateUser.data.image);
        });
    });
